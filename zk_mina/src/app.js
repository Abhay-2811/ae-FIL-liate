...
const start = async () => {
  ...
  const { privateKey: userKey1, publicKey: userAccount1 } =
    Local.testAccounts[1];
  const { privateKey: userKey2, publicKey: userAccount2 } =
    Local.testAccounts[2];
  const { privateKey: userKey3, publicKey: userAccount3 } =
  ...
  app.get('/states', (req, res) => {
    ...
    res
      ...
      .send(
        JSON.parse(
          `{"p_status": ${p_status}, "p_unconfirmed_status": ${p_unconfirmed_status}}`
        )
      );
  });
  ...
  app.post('/userUpdateMainState', async (req, res) => {
    try {
      const txn2 = await Mina.transaction(userAccount1, () => {
        zkAppInstance.proveByUser(
          zkAppInstance.p_state.get(),
          Field(req.body.git),
          Field(req.body.biz)
        );
      });
