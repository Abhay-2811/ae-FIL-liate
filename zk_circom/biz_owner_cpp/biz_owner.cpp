...
void githubVerify_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void githubVerify_3_run(uint ctx_index,Circom_CalcWit* ctx);
void bizAge_4_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void bizAge_4_run(uint ctx_index,Circom_CalcWit* ctx);
void aFILliate_5_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void aFILliate_5_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 42;}
...
ctx->componentMemory[coffset].templateName = "githubVerify";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 1;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "gtDays";
...
Fr_eq(&expaux[0],&signalValues[mySignalStart + 0],&circuitConstants[2]); // line circom 16
...
ctx->componentMemory[coffset].templateName = "bizAge";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 1;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
uint csoffset = mySignalStart+2;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "bizMonths";
GreaterEqThan_2_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 17 ;
aux_cmp_num += 3;
...
Fr_eq(&expaux[0],&signalValues[mySignalStart + 0],&circuitConstants[2]); // line circom 34
...
ctx->componentMemory[coffset].templateName = "aFILliate";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "git";
githubVerify_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 19 ;
aux_cmp_num += 4;
}
...
uint csoffset = mySignalStart+3;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "biz";
bizAge_4_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 19 ;
aux_cmp_num += 4;
}
...
// need to run sub component
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1));
githubVerify_3_run(mySubcomponents[cmp_index_ref],ctx);
}
{
...
// need to run sub component
assert(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1));
bizAge_4_run(mySubcomponents[cmp_index_ref],ctx);
}
{
PFrElement aux_dest = &lvar[0];
// load src
Fr_mul(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[1]].signalStart + 0]); // line circom 52
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_eq(&expaux[0],&lvar[0],&circuitConstants[2]); // line circom 54
...
aFILliate_5_create(1,0,ctx,"main",0);
aFILliate_5_run(0,ctx);
}
